...
namespace PsiBot.Service.Settings
{
    ...
    public class BotConfiguration
    {
        ...
        public string TextBotId { get; set; }
        public string TextBotSecret { get; set; }
        public string BotConnectorUrl { get; set; }
        public string BotConnectorAuthUrl { get; set; }
        public MTConfiguration Translation { get; set; }
        public string DirectLineSecret { get; set; }
        public string OffDirectLineUrl { get; set; }
        public string TranscriptFolder { get; set; }
        public string BrowserRedirectUrl { get; set; }
        ...
        public X509Certificate2 GetCertificateFromStore()
        {
            ...
            X509Store altstore = new X509Store(StoreName.My, StoreLocation.CurrentUser); //for azure
            store.Open(OpenFlags.ReadOnly);
            altstore.Open(OpenFlags.ReadOnly);
            try
            {            
                ...
                if (certs.Count < 1)
                {
                    X509Certificate2Collection altcerts = altstore.Certificates.Find(X509FindType.FindByThumbprint, CertificateThumbprint, validOnly: false);
                    if (altcerts.Count >= 1)
                    {                        
                        return altcerts[0];
                    }
